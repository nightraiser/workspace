...
move_uploaded_file($_FILES["file"]["tmp_name"], "../assets/products/" . $newfilename) or die("error");
    chmod("../assets/products/$newfilename", 0766) or die("error");
    ...
    function category()
    {
         $.post("framework/requests.php",{products:<?php echo $_GET['scid']; ?>},function (data) 
        {
            ...
            var i=1;
                for(k in d)
                {
                     output+="<tr><td>"+i+"</td><td>"+d[k]['product_name']+"</td><td><a onclick='return del(this);'id="+d[k]['pid']+" class='btn btn-danger'>Delete</a></td></tr>";
                     i++;
                 }
                 $("#tbody").html(output);
             }
