...
var cur = <?php echo"$_GET[pid]"; ?>;
localStorage.setItem("pid",prev+","+cur);
var quant = $("#qty").val();
var prevq = localStorage.getItem("qty");
localStorage.setItem("qty",prevq+","+quant);
$("#add_to_cart").attr("class","btn btn-larg pull-right btn-success");
$("#add_to_cart").html('<i class=" icon-shopping-cart"></i> Added to cart');
return false;
}
</script>
<h2>Product</h2>
				...
				<form  class="form-horizontal qtyFrm" action ="add_to_cart.php" method="post">
				  <div class="control-group">
					...
					<input type="number" min="1" max="5" id="qty"class="span1" value="1"/>
					  <button onclick="addtocart();"  class="btn btn-large pull-right btn-info" id="add_to_cart" ><i class=" icon-shopping-cart"></i> Add to cart</button>
					</div>
				  </div>
				</form>
