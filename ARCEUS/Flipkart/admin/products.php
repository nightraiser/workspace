    ...
    function category()
    {
         $.post("framework/requests.php",{products:<?php echo $_GET['scid']; ?>},function (data) 
        {
            ...
            var i=1;
                for(k in d)
                {
                     output+="<tr><td>"+i+"</td><td>"+d[k]['product_name']+"</td><td><a href=products.php?scid="+d[k]['cid']+" class='btn btn-success'>Add</a></td><td><a onclick='return del(this);'id="+d[k]['cid']+" class='btn btn-danger'>Delete</a></td></tr>";
                     i++;
                 }
                 $("#tbody").html(output);
             }
