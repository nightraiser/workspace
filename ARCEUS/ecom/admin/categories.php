...
<div class="container">
 ...
 <script>
    ...
    function category()
    {
         $.post("framework/requests.php",{category:"all"},function (data) 
        {
            if(data!=false)
            {
            ...
            var i=1;
                for(k in d)
                {
                     output+="<tr><td>"+i+"</td><td>"+d[k]['category_name']+"</td><td><a href=subcategories.php?cid="+d[k]['cid']+" class='btn btn-success'>Add</a></td><td><a onclick='return del(this);'id="+d[k]['cid']+" class='btn btn-danger'>Delete</a></td></tr>";
                     i++;
                 }
                 $("#tbody").html(output);
             }
    ...
    function del(button)
    {
        $.post("framework/requests.php",{remove:button.id,type:"cid"},
            function (data)
            {
                if(data==true)
                {
